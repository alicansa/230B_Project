...
SNR = 100; %SNR levels where the system will be simulated
...
figure(1)
plot(t_analog(1:100*overSampleSizeAnalog/overSampleSize),real(filtered_transmit_analog(1:100*overSampleSizeAnalog/overSampleSize)),'-b');


ylabel('Real part of signal');
xlabel('time (sec)');
legend('D/A output',...
    'Original Digital Signal','Anti-aliasing filter output'...
    );
print(h,'-djpeg','-r300','DtoA');
...
for i=1:length(SNR)
 %pass the signals to be transmitted through awgn channel
    received_analog = awgn_complex_channel(filtered_transmit_analog,SNR(i),5*S);
    ...
    filtered_received_analog = ButterworthFilter(4,0.3,received_analog); %fc at pi/5
    ...
    figure(2)
    plot(t_digital(1:100),real(received_digital(1:100)));
    
    
    ylabel('Real part of signal');
    xlabel('time (sec)');
    legend('Original Received Analog signal','Noise-limiting filter output',...
    'A/D output'...
    );
    print(h,'-djpeg','-r300','AtoD');
