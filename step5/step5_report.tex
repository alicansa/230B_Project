...
However this step doesn't require us to estimate the channel. It is assumed that we the channel reponse is estimated before. The following bandlimited channel responses are used in this step of the project:
...
Equalization of the channel is chosen from a variety of techniques to accomplish the cancelation of delayed verions of the symbols. 

The equalizers tried out in this step, to achieve delayed symbol cancelation are:
...
The results of the experiment done for each equalizers over the three channels are given in the previous section. From the BER and constellation plots obtained it can be seen that each equalizer does a good job of cancelation of the delayed symbols. The BER of the system with QPSK and BPSK modulations are almost the same as theoretical results when the channel is equalized. 

As expected, out of these three equalizers MMSE-DFE is the best, MMSE is the second best and ZF is the last. This is caused by the fact that in MMSE based equalizers, the effect of AWGN noise is considered. On the other hand ZF equalization doesn't take the channel noise into account which in the end causes noise enhancement. 

When the equalizer has a feedback loop, the error enhancement issue is addressed. This balances the need to eliminate ISI and the danger of noise enchancement. Therefore we see the best results with MMSE-DFE equalizer.

Still one can use ZF or MMSE equalizer with more taps to achieve the same BER results instead of using MMSE-DFE equalizer which increases the complexity of the overall system. Yet this is a trade-off where an engineer has to decide on choosing between more memory or complexity of the feedback loop.
