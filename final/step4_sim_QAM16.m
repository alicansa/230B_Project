%Simulation for the QAM16 modulation scheme
close all;
clearvars;
clc;
...
[quadrature, inphase] =  QAM_16_mod(bits,N/k);  %mapping to symbols
...
for i=1:length(SNR)
   %pass the signals to be transmitted through awgn channel
    ...
    %initialize feedback parameters
       ...
       %   print(f,'-djpeg','-r300','loop_filter_16QAM5');
...
%             sampled_equalized = conv(sampled,c);            
 %           sampled_equalized = sampled_equalized((floor(L/2))+1:end-(floor(L/2)));
            ...
            [output_bits] = QAM_16_demod(real(sampled),...
                imag(sampled));
